...
struct BMP {
    ...
    void dump_rgba_data(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h, framebuffer::RGBType *rgbdata) {
        if (x0 + w > (uint32_t)bmp_info_header.width || y0 + h > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The region does not fit in the image!");
        }
        ...
        for (uint32_t y = y0; y < y0 + h; ++y) {
            ...
            {
                int i = ((y*w) + x);
                framebuffer::RGBType rgb = rgbdata[i];

                uint8_t R  = (rgbdata[i].r);
                uint8_t G  = (rgbdata[i].g);
                uint8_t B  = (rgbdata[i].b);
                uint8_t A  = 255;
